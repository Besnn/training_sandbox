#!/usr/bin/env python3
"""Evaluate YOLOv8-OBB and YOLO26-OBB ONNX models.
...
Usage:
    python3 evaluate-yolo-obb.py
    python3 evaluate-yolo-obb.py \
        --images <dir> --labels <dir> \
        --model "YOLOv8-OBB=models/yolov8n/yolov8n-obb-fp32.onnx" \
        --model "YOLO26-OBB=models/yolo26n/yolo26n-obb-fp32.onnx"
"""
...
def print_per_class(name, result):
    ...
    n_gt = result["n_gt"]
    ap = result["ap"]
    print(f"\n--- {name} per-class metrics (IoU=0.5 for P/R/F1) ---")
    header = f"{'class':<22}{'GT':>6}{'P':>10}{'R':>10}{'F1':>10}{'AP50':>10}{'AP50-95':>10}{'conf*':>10}"
    print(header)
    print("-" * len(header))
